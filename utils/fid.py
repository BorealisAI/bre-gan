...
'''
# code taken from https://github.com/bioinf-jku/TTUR
...
def calculate_fid_given_imgs(imgs, inception_path):
    ...
    with tf.Session() as sess:
        ...
        m1, s1 = _handle_imgs(imgs[0], sess)
        m2, s2 = _handle_imgs(imgs[1], sess)
        fid_value = calculate_frechet_distance(m1, s1, m2, s2)
        return fid_value


def _get_img_hash(imgs):
    import hashlib
    m = hashlib.md5()
    m.update(repr(imgs))
    return m.hexdigest()
...
def calculate_fid_with_ref_imgs(imgs, ref_imgs, inception_path):
    ''' Calculates the FID of two paths. '''
    inception_path = check_or_download_inception(inception_path)
    ...
    with tf.Session() as sess:
        ...
        m1, s1 = _handle_imgs(imgs, sess)

        del imgs
        import gc
        gc.collect()

        ref_name = _get_img_hash(ref_imgs) + '.npz'
        # import ipdb
        # ipdb.set_trace()

        if os.path.exists(ref_name):
            print('loading inception stats from {}'.format(ref_name))
            f = np.load(ref_name)
            m2, s2 = f['mu'][:], f['sigma'][:]
            f.close()
        else:
            m2, s2 = _handle_imgs(ref_imgs, sess)
            np.savez_compressed(ref_name, mu=m2, sigma=s2)
            print('saving inception stats to {}'.format(ref_name))
