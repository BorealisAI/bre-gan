...
def load_ref_data(filename):
    if 'cifar' in filename.lower():
        ...
        data = datasets.get_celeba()
    else:
        raise ValueError()
    return data


def fid_with_ref(filename, ref_imgs=None):
...
if __name__ == '__main__':
    ...
    if 'data_file' in kwargs:
        ...
        ref_data = load_ref_data(filename)
        score = fid_with_ref(filename, ref_data)
        ...
        ref_data = load_ref_data(file_names[0])

        scores = [fid_with_ref(f, ref_data) for f in file_names]
        ...
        with open(result_file+'results.csv', 'w') as fw:
            if notes:
                fw.write('filename;\t score;\t notes \n')
                for l in results:
                    fw.write('{};\t {};\t {}\n'.format(
                        l[0], l[1], l[2]))
            else:
                fw.write('filename;\t avg \n')
                for l in results:
                    fw.write('{};\t {}\n'.format(l[0], l[1]))
